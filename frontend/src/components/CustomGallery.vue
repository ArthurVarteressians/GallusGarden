     ...
     <div class="h-screen flex flex-col items-center">
       ...
       <main class="w-[80vw] h-[80vh] overflow-y-auto p-4">
         <div class="space-y-4">
           ...
           <div v-for="(row, index) in generateRows()" :key="index" :class="getRowClass(index)">
             <div
               v-for="image in row"
               :key="image.publicId"
               :class="getImageClass(index, row.indexOf(image))"
             >
               <img :src="image.url" :alt="image.description" class=" w-[100vw] h-[20vh] rounded-lg object-cover" />
             </div>
           </div>
         </div>
         <!-- Loading Indicator -->
   ...
   export default defineComponent({
     ...
     setup() {
       ...
       const getRowClass = (index: number) => {
         return index % 2 === 0 ? 'grid grid-cols-4 gap-4' : 'grid grid-cols-4 gap-4'; // Both rows use a grid layout
       };
